...
qry_str = ("""
    ...
    select date_UTC, CONCAT(cast(a.pod_id_location as STRING),'_a') as pod_str, no2_ppb
...
left join cte1 on cte0.date_UTC = cte1.mobiletime) group by hour1stamp, pod_str, segid)) 
    ...
    ,percentile_cont(bias_pct,0.05) OVER (partition by pod_str, n_passes) as p05
    ...
    ,percentile_cont(bias_pct,0.95) OVER (partition by pod_str, n_passes) as p95
    from cte2
    )
...
plt1.save(filename = r'.\charts\bias.png', width=None, height=None, dpi=200)
#n_segments
...
plt2.save(filename = r'.\charts\n_segments.png', width=None, height=None, dpi=200)
