...
SELECT datetime_sub(timestamp, INTERVAL if(a.vid=27522 and a.parameter_id=133 and timestamp>='2019-05-01 16:20:00' and timestamp<'2019-06-03 22:56:00',6,if(timestamp < ifnull(filter_change_date,timestamp), pre_filter_alignment_s,if(timestamp < ifnull(palas_change_date,timestamp), post_filter_alignment_s,ifnull(post_palas_alignment_s,0)))) SECOND) AS timestamp_aligned
...
cte4 AS (
SELECT vid, timestamp, value AS satellite_tracking 
FROM {0}
where parameter_id = 150
)
...
,cte1.latitude, cte2.longitude, cte3.speed_kmh, cte4.satellite_tracking, z.clock_diff_s, z.clock_diff_s*cte3.speed_kmh*0.277778 as clock_spatial_uncertainty_m
FROM cte0
...
LEFT JOIN cte4 ON cte0.vid = cte4.vid AND cte0.timestamp_aligned = cte4.timestamp
