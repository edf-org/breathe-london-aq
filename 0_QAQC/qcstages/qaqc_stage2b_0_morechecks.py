...
def limit_min():
    min_thresh_dict = {0:{'val':-1,'p':[140],'unit':'ug m-3'}\
            ...
            ,7:{'val':-10,'p':[32,33],'unit':'umol/mol'}\
            ,8:{'val':60,'p':[34],'unit':'mmol/mol'}\
...
#latitude and longitude checks (not within greater london or at airmonitors)
#and where any coords are null 
...
#not implemented because start and stop times of precip too vague
...
#missing span or zero check
#na, final checks completed at end of project
#def set_missing_span_zero():
...
def set_span_outofrange():
    qry_str = ("""
    select car_id, date,measure, span_actual, span_cert from 
    (select car_id, date, measure
    ...
    for row in qry_job.result():
        ...
        qry_job = bqclient.query(qry_str,location='eu',job_config=bigquery.QueryJobConfig())
        rows = qry_job.result()
        print("qc flags updated in table {0} for {4} {1} span out of range between {2} and {3}. Target: {6} Actual: {5}".format(destinationtable_str,row.measure,lastvalid,discoveredinvalid,row.car_id,row.span_actual, row.span_cert))
...
def flag_known_leaks():
    ...
    and timestamp < '2019-06-06 9:18:00'
    """.format(destinationtable_str))
    qry_job = bqclient.query(qry_str,location='eu',job_config=bigquery.QueryJobConfig())
    rows = qry_job.result()
    print("query results loaded into table {0}".format(destinationtable_str))

    #period of pump failure
    qry_str = ("""update {0} a
    set qc_flag = 94
    where vid = 27522 and parameter_id = 133 --no2
    and timestamp >= '2019-09-19' 
    """.format(destinationtable_str))
    qry_job = bqclient.query(qry_str,location='eu',job_config=bigquery.QueryJobConfig())
    rows = qry_job.result()
    print("query results loaded into table {0}".format(destinationtable_str))
    
    #last calibration for no on 27522 was Aug 21 2019
    #known pump failure occurred between last cal and end of collection
    qry_str = ("""update {0} a
    set qc_flag = 94
    where vid = 27522 and parameter_id = 45 --no
    and timestamp > '2019-08-21' 
    and not STARTS_WITH(cast(qc_flag as string),'9')
    """.format(destinationtable_str))
    qry_job = bqclient.query(qry_str,location='eu',job_config=bigquery.QueryJobConfig())
    rows = qry_job.result()
...
def flag_biased_pm25():
    qry_str = ("""update {0} a
    set qc_flag = if((qc_flag in (0,28)) or (qc_flag is null), 28, 27) --if not already suspect 28 otherwise 27
    where parameter_id in (3,140)
    and not STARTS_WITH(cast(qc_flag as string),'9') --don't overwrite anything invalid
        and not qc_flag in (4,5,6,7,10) --or anything other than valid and suspect
    """.format(destinationtable_str))
    qry_job = bqclient.query(qry_str,location='eu',job_config=bigquery.QueryJobConfig())
    rows = qry_job.result()
    print("query results loaded into table {0}".format(destinationtable_str))
...
def flag_spatial_resolution(lim):
    qry_str = ("""update {0} a
    set qc_flag = if((qc_flag in (0,25)) or (qc_flag is null), 25, 27) --if not already suspect 25 otherwise 27
...
def flag_gps_accuracy(num_track_sats):
    qry_str = ("""update {0} a
    set qc_flag = if((qc_flag in (0,26)) or (qc_flag is null), 26, 27) --if not already suspect 26 otherwise 27
    where satellite_tracking < {1}
    and not STARTS_WITH(cast(qc_flag as string),'9') --don't overwrite anything invalid
        and not qc_flag in (4,5,6,7,10) --or anything other than valid and suspect
    """.format(destinationtable_str, num_track_sats))
    qry_job = bqclient.query(qry_str,location='eu',job_config=bigquery.QueryJobConfig())
    rows = qry_job.result()
    print("query results loaded into table {0}".format(destinationtable_str))
...
limit_temp([133],0,45)
print('flagging temperatures outside 0-50 C for palas fidas')
limit_temp([1,2,3,4,5,6,7,15,16,17,160,161,162],0,50)
