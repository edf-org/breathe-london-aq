...
#default coordinates for NPL facility overall
#don't overwrite an existing mode
def set_mobile(n=51.426999,s=51.4235,e=-0.341565,w=-0.346812):
    #not on road
    qry_str = ("""update {0} a
    set mmode = 1
    where 
...
#bounding box around NPL parking lot
#don't overwrite an existing mode
...
#measurement mode 5
#Other uncategorized close to NPL but not obviously in parking lot. May be useful in analyses of stationary data however GPS coordinates uncertain. 
def set_other():
    #not assigned to another mode
    qry_str = ("""update {0} a
    set mmode = 5
    where 
    mmode is null
