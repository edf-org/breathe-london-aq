...
(qc_flag in (0,2,20,21,22,23,24,28)) and (mmode=1 or mmode=2))
    """.format(srctable_str))
...
job_config.destination = dataset_ref.table(destinationtable_str)
def qry_str(vid,time_str,batchid): 
    return """
...
--this is fast but the sort operations in the analytic over clauses can cause memory exceedance
--so we break up by vid and time period
with cte as 
(
select a.pid
...
where vid = {0} and timestamp {1} and mindist_m <= 30 -- snap only points 30 m or less away
group by pid
...
concat('{2}','_',cast(sum(IF(sq.preceding_segid = sq.segid,0,1)) over(partition by vid, segid order by sq.timestamp) as string)) as passid
from (
    ...
    from cte
        ...
        """.format(vid, time_str,batchid)
#run in parts
#first overwrites existing
qry_job = client.query(qry_str(27522,"< '2019-03-03'",1),location=location,job_config=job_config)
rows = qry_job.result()
#subsequent append
job_config.write_disposition = 'WRITE_APPEND' #append existing
qry_job = client.query(qry_str(27522,">= '2019-03-03'",2),location=location,job_config=job_config)
rows = qry_job.result()
qry_job = client.query(qry_str(27533,"< '2019-03-03'",1),location=location,job_config=job_config)
rows = qry_job.result()
qry_job = client.query(qry_str(27533,">= '2019-03-03'",2),location=location,job_config=job_config)
rows = qry_job.result()
print("Query results loaded into table {0}".format(destinationtable_str))
...
on b.pid = c.pid
where (a.qc_flag in (0,2,20,21,22,23,24,28)) and a.value is not null and a.mmode = 1
)
--then make drive pass averages, grouping by vid, segment, and passid
...
join UK.snap2_ptclosestroad_lookup c
on b.pid = c.pid
where (a.qc_flag in (0,2,21,22,23,24,28)) and a.value is not null and a.mmode = 1
)
